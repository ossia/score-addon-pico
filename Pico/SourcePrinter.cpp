...
std::string address_to_device_read_index(const State::Address& addr)
{
  if (addr.path.size() < 2)
    return error_string;
...
std::string address_to_network_read_index(const State::Address& addr)
{
  return 0;
}
std::string
...
std::string address_to_network_write_index(const State::Address& addr)
{
  return 0;
}

QString SourcePrinter::printTask(
    const Device& device,
...
QString SourcePrinter::printDeviceInitialization(const Device& device)
{
  QString str;
